...
exports.submitOtp = exports.initiateCheckIn = void 0;
...
// ── Callable Functions ─────────────────────────────────────────────────────
exports.initiateCheckIn = functions.https.onCall(async (data, _context) => {
    // ── Input validation ──────────────────────────────────────────────────
    const { lockerId, email } = data;
    if (!lockerId || typeof lockerId !== "string" || lockerId.trim() === "") {
        throw new functions.https.HttpsError("invalid-argument", "lockerId is required");
    }
    ...
    try {
        const result = await db.runTransaction(async (txn) => {
            ...
            // Check isOnline
            if (!locker.isOnline) {
                throw new functions.https.HttpsError("failed-precondition", "LOCKER_OFFLINE");
            }
            // Check state === EMPTY
            if (locker.state !== "EMPTY") {
                throw new functions.https.HttpsError("failed-precondition", "LOCKER_NOT_AVAILABLE");
            }
            // Check no concurrent transaction (race condition guard)
            if (locker.activeTransactionId !== null &&
                locker.activeTransactionId !== undefined) {
                throw new functions.https.HttpsError("failed-precondition", "LOCKER_CONFLICT");
            }
            ...
            const otpExpiresAt = admin.firestore.Timestamp.fromMillis(now.toMillis() + OTP_EXPIRY_HOURS * 60 * 60 * 1000);
...
exports.submitOtp = functions.https.onCall(async (data, _context) => {
    ...
    if (!isValid) {
        ...
        const result = await db.runTransaction(async (txn) => {
            ...
            if (newFailedAttempts >= 5) {
                ...
                const lockedUntilTimestamp = admin.firestore.Timestamp.fromMillis(lockedUntilMs);
                txn.update(lockerRef, {
                    failedOtpAttempts: 0,
    ...
    try {
        ...
        functions.logger.error("Failed to write UNLOCK command to RTDB during check-out", err);
    ...
    try {
        ...
        functions.logger.error("Failed to update Firestore during check-out", err);
        throw new functions.https.HttpsError("internal", "Failed to complete check-out");
    }
    // d. Return success
    return {
        success: true,
